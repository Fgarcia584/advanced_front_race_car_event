...
const RacesMap = () => {
    ...
    const center : [number, number] = [coords?.latitude ?? 50.6311278, coords?.longitude ?? 3.0056985 ]; 
        ...
        <MapContainer className="h-screen m-10" zoom={13} scrollWheelZoom={true}>
            ...
            >
                {state.races.map((race) => (
                    <Marker id={race.id} position={[race.latitude, race.longitude]} icon={customIcon}>
